...
class Solution(object):
    def maxVacationDays(self, flights, days):
        ...
        for i in range(k):
            next_week = [float('-inf')] * n
            for _from in range(n):
                for _to in range(n):
                    if _from == _to or flights[_from][_to]:
                        next_week[_to] = max(next_week[_to], dp[_from] + days[_to][i])
            dp = next_week
...
if __name__ == "__main__":
    ...
    assert(sol.maxVacationDays(flights, days) == 12)

    flights = [[0, 0, 0], [0, 0, 0], [0, 0, 0]]
    days = [[1, 1, 1], [7, 7, 7], [7, 7, 7]]
    assert(sol.maxVacationDays(flights, days) == 3)

    flights = [[0, 1, 1], [1, 0, 1], [1, 1, 0]]
    days = [[7, 0, 0], [0, 7, 0], [0, 0, 7]]
    assert(sol.maxVacationDays(flights, days) == 21)
